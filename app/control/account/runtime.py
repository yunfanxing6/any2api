...
def reconcile_refresh_runtime(enabled: bool | None = None) -> Literal["quota", "random"]:
    ...
    refresh_enabled = config.get_bool("account.refresh.enabled", False) if enabled is None else bool(enabled)
    target_strategy: Literal["quota", "random"] = "quota" if refresh_enabled else "random"

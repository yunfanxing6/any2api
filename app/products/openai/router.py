...
def _qwen_provider(request: Request):
...
def _request_allowed_providers(request: Request) -> set[str] | None:
    record = getattr(request.state, "api_key_record", None) or {}
    providers = record.get("providers") if isinstance(record, dict) else None
    if not providers:
        return None
    return {str(item).strip().lower() for item in providers if str(item).strip()}
...
async def _model_available(request: Request, spec: ModelSpec) -> bool:
    allowed_providers = _request_allowed_providers(request)
    provider = _model_provider(spec.model_name)
...
def _require_qwen_provider(request: Request):
    provider = _qwen_provider(request)
...
async def list_models(request: Request):
    ...
    for m in model_registry.list_enabled():
        if not await _model_available(request, m):
            continue
        models.append(
            {
                "id":       m.model_name,
...
async def get_model_endpoint(model_id: str, request: Request):
    ...
    if spec is None or not await _model_available(request, spec):
        return JSONResponse(
...
# ---------------------------------------------------------------------------
# /v1/files (Qwen passthrough)
# ---------------------------------------------------------------------------

@router.post("/files", tags=[_TAG_FILES], dependencies=[Depends(verify_api_key)])
async def upload_file_endpoint(request: Request):
    provider = _require_qwen_provider(request)
    body = await request.body()
    return await provider.forward_content(
        "POST",
        "/v1/files",
        content=body,
...
@router.delete("/files/{file_id}", tags=[_TAG_FILES], dependencies=[Depends(verify_api_key)])
...
async def responses_endpoint(req: ResponsesCreateRequest, request: Request):
    ...
    if _is_qwen_model(req.model):
        raise _ValidationError(
            "Qwen models do not support the Responses API in this release.",
            param="model",
